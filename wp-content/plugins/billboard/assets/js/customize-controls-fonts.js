/*global _:false, _billboardFontsControlsSettings:false, Backbone:false, WebFont:false, wp:false */
...
(function( window, $, _, Backbone, wp, undefined ) {
	...
	var api = wp.customize,
		app = {},
		settings = _billboardFontsControlsSettings;
	...
	app.view.FontControl = wp.Backbone.View.extend({
		template: wp.template( 'billboard-fonts-control-font' ),
		...
		render: function() {
			...
			this.views.add( '.billboard-fonts-control-content', [
				new app.view.FontDropdown({
					collection: this.collection,
					control: this.options.control
				})/*,
				new app.view.FontSizeSlider({
					control: this.options.control,
				})*/
			]);
	...
	app.view.FontDropdown = wp.Backbone.View.extend({
		className: 'billboard-fonts-dropdown',
		tagName: 'div',
		...
		updateDropdownPositionAndHeight: function() {
			var topSpace,
				...
				$list = this.$el.find( '.billboard-fonts-list' ),
	...
	app.view.FontDropdownToggle = wp.Backbone.View.extend({
		className: 'billboard-fonts-dropdown-toggle',
		tagName: 'span',
	...
	app.view.FontDropdownResetButton = wp.Backbone.View.extend({
		className: 'billboard-fonts-dropdown-reset-button',
		tagName: 'button',
	...
	app.view.FontList = wp.Backbone.View.extend({
		className: 'billboard-fonts-list',
		tagName: 'ul',
	...
	app.view.FontListItem = wp.Backbone.View.extend({
		className: 'billboard-fonts-list-item',
		tagName: 'li',
	...
	app.view.FontSizeSlider = wp.Backbone.View.extend({
		tagName: 'p',
		template: wp.template( 'billboard-fonts-control-font-size-slider' ),

		events: {
			'input input': 'updateOutput'
		},

		render: function() {
			var data = {
					id: this.options.control.id,
					size: 16
				};
		...
		updateOutput: function() {
			var value = this.$input.val(),
				state = this.options.control.state;

			this.$output.val( value );
			state.get( 'font' ).set( 'size', value );
		}
	});
	...
	api.sectionConstructor['billboard-fonts'] = api.Section.extend({
		...
		attachEvents: function() {
			...
			$buttons = this.container.find( '.billboard-fonts-section-toggle' );
			$content = this.container.find( '.billboard-fonts-section-content' );
			...
			$( '#billboard-fonts-option-typekit-id' ).on( 'change', this.updateTypekitId );
		...
		maybeLoadFonts: function() {
			var typekitKitId = api( 'billboard_fonts[typekit_id]' )();
		...
		updateTypekitId: function( e ) {
			...
			if ( '' === $field.val() ) {
				api( 'billboard_fonts[typekit_id]' ).set( '' );
				app.fonts.trigger( 'reset' );
				return;
			}
			...
			app.loadTypekitFonts( $field.val() )
				.done(function( response ) {
					api( 'billboard_fonts[typekit_id]' ).set( $field.val() );
				})
				.always(function() {
	...
	api.controlConstructor['billboard-font'] = api.Control.extend({
		ready: function() {
