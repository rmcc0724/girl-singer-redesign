...
methods = {
	...
	addItem: function addItem() {
		...
		repeater.find('.audiotheme-repeater-items').append(itemTemplate.clone()).children(':last-child').find('input,select,textarea').each(function () {
			var $this = $(this);
			$this.attr('name', $this.attr('name').replace('[0]', '[' + itemIndex + ']'));
		}).end().find('.audiotheme-clear-on-add').val('').end().find('.audiotheme-remove-on-add').remove().end().find('.audiotheme-show-on-add').show().end().find('.audiotheme-hide-on-add').hide().end();
...
$.fn.audiothemeRepeater = function (method) {
	...
	} else if ('object' === (typeof method === 'undefined' ? 'undefined' : _typeof(method)) || !method) {
