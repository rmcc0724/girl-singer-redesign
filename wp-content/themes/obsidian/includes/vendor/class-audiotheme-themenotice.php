...
class AudioTheme_ThemeNotice {
	...
	public function __construct( $args = array() ) {
		if ( ! $this->is_audiotheme_active() && is_admin() && current_user_can( 'activate_plugins' ) ) {
			$this->strings = array(
				'notice'     => esc_html__( 'The AudioTheme plugin should be installed and activated for this theme to display properly.', 'obsidian' ),
				'activate'   => esc_html__( 'Activate now', 'obsidian' ),
				'learn_more' => esc_html__( 'Find out more', 'obsidian' ),
				'dismiss'    => esc_html__( 'Dismiss', 'obsidian' ),
			);
