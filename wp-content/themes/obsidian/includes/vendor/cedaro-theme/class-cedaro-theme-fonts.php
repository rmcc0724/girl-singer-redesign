...
class Cedaro_Theme_Fonts {
	 ...
	 * @return array
	 */
	public function register_resource_hints( $urls, $relation_type ) {
		if ( 'preconnect' !== $relation_type ) {
			return $urls;
		}

		if ( wp_style_is( 'cedaro-theme-fonts-google', 'queue' ) ) {
			$urls[] = array(
		...
		if ( wp_style_is( 'cedaro-theme-fonts-typekit', 'queue' ) ) {
			$urls[] = array(
	...
	public function body_classes( $classes ) {
		foreach ( $this->text_groups as $group ) {
			...
			$family  = preg_replace( '/[^a-z0-9-]/', '', strtolower( str_replace( ' ', '-',  $family ) ) );
			$classes[] = 'font_' . $group['id'] . '_' . $family;
		}
	...
	public function customize_register( $wp_customize ) {
		...
		$wp_customize->add_section( new Cedaro_Theme_Customize_Section_Fonts( $wp_customize, 'cedaro_fonts', array(
			'title'       => esc_html__( 'Fonts', 'obsidian' ),
			'priority'    => 50,
		) ) );
	...
	public function enqueue_customizer_controls_assets() {
		...
		wp_localize_script( 'cedaro-theme-customize-controls-fonts', '_cedaroThemeFontsControlsSettings', array(
			'fonts' => $this->fonts,
			'l10n'  => array(
				'reset'       => esc_html__( 'Reset', 'obsidian' ),
				'defaultFont' => esc_html__( 'Default Theme Font', 'obsidian' ),
			),
		) );
	}
	...
	protected function register_default_fonts() {
		$this->add_fonts( array(
			...
			array( 'family' => 'Libre Franklin',     'stack' => '"Libre Franklin", sans-serif',   'tags' => array( 'content', 'heading' ) ),
	...
	protected function get_subsets() {
		 ...
		 */
		$subset = esc_html_x( 'no-subset', 'Add new subset (latin-ext)', 'obsidian' );
