/*global _:false, _cedaroThemeFontsPreviewSettings:false, Backbone:false, wp:false */
...
(function( window, $, _, Backbone, wp, undefined ) {
	...
	var api = wp.customize,
		app = {},
		loadedFonts = [],
		settings = _cedaroThemeFontsPreviewSettings;
	...
	api.bind( 'preview-ready', function() {
		...
		_.each( settings.groups, function( group ) {
			...
			api( group.id + '_font', function( setting ) {
				setting.bind(function( value, from ) {
					var familyId;;

					if ( from ) {
						familyId = from.family.toLowerCase().replace( /\s/g, '-' ).replace( /[^a-z0-9-]/, '' )
						$body.removeClass( 'font_' + group.id + '_' + familyId );
					}

					if ( value ) {
						familyId = value.family.toLowerCase().replace( /\s/g, '-' ).replace( /[^a-z0-9-]/, '' );
						$body.addClass( 'font_' + group.id + '_' + familyId );
					}
